...
# there is no application object when writing the rest of your code. get_db will be called when the application has been created and is handling a request,
...
#
# close_db checks if a connection was created by checking if g.db was set. If the connection exists, it is closed. Further down you will tell your application about the
# close_db function in the application factory so that it is called after each request.
def init_db():
    db = get_db()
